...
本项目仅供学习与技术参考，不构成生产部署方案。运行过程中会调用 Amazon Bedrock 上的 AI 模型并产生费用，请根据实际使用量评估成本。作者不对因使用本项目产生的任何费用或损失承担责任。本项目与 Amazon Web Services 及 Moonshot AI 无官方关联，相关服务的可用性与定价以各方官方文档为准。生产环境使用前请根据实际需求进行安全评估与调整。
